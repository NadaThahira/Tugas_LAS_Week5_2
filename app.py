...
def render_diagnosis():
    ...
    if st.session_state.stage == 1:
        ...
        with st.expander("Penyakit yang dapat dikenali"):
            st.markdown(
                '<div style="font-size:0.88rem; color:{}; margin-bottom:0.8rem;">'
                'Mencakup berbagai penyakit pada daun tomat dengan tingkat keparahan yang beragam.'
                '</div>'.format(t["muted"]),
                unsafe_allow_html=True,
            )
            ...
            for cls in CLASSES:
                ...
                tag = (
                    ""
                    if info["tingkat"] == "sehat"
                    else f'<span style="font-size:0.72rem; color:{color} !important; '
                         f'font-weight:600;"> · {info["tingkat"].upper()}</span>'
                )
        ...
        st.caption(
            "TomaLeaf Dx · Model: CNN Custom · Nada Thahira Sosa — 2601 · MBC Lab Week 2"
        )
        cards = []
        for cls in CLASSES:
            info = DISEASE_INFO[cls]
